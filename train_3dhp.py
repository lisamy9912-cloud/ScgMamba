...
from models.model.loss import loss_mpjpe, n_mpjpe, loss_velocity, loss_limb_var, loss_limb_gt, loss_angle, \
    loss_angle_velocity

from models.utils.data import denormalize

from mipdata.reader.motion_dataset import MPI3DHP, Fusion

from models.utils.tools import set_random_seed, get_config, print_args, ensure_dir
from torch.utils.data import DataLoader

from models.utils.learning import AverageMeter, decay_lr_exponentially,load_model_mamba
from models.utils.tools import count_param_numbers
from models.utils.utils_3dhp import *
os.environ['CUDA_VISIBLE_DEVICES'] = '0' 

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, default="configs/config/ScgMamba_train_3dhp.yaml", help="Path to the config file.")
    parser.add_argument('-c', '--checkpoint', type=str, metavar='PATH',
                        help='checkpoint directory')
    parser.add_argument('--checkpoint-file', type=str, help="checkpoint file name")
    parser.add_argument('--new-checkpoint', type=str, metavar='PATH', default='checkpoint/mpi/ScgMamba_L_81',
...
def train(args, opts):
    print_args(args)
    ensure_dir(opts.new_checkpoint)
    log_file = os.path.join(opts.new_checkpoint, 'log.txt')
