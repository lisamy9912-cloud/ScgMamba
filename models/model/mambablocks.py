...
class GhostMotionMLP(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., bias=True):
        ...
        self.primary_dim = hidden_features // 2
        self.ghost_dim = hidden_features - self.primary_dim

        self.fc1_primary = nn.Linear(in_features, self.primary_dim, bias=bias)

        self.fc1_ghost = nn.Conv2d(
            self.primary_dim, self.ghost_dim,
            kernel_size=(3, 1), padding=(1, 0),
            groups=self.primary_dim, bias=bias
        )
    ...
    def forward(self, x):
        # Input: (B, F, 1, C)
        x_smooth = x.permute(0, 3, 1, 2)  # (B, C, F, 1)
        x_smooth = self.dwconv_pre(x_smooth)
        x_smooth = x_smooth.permute(0, 2, 3, 1)  # (B, F, 1, C)
        x = x + x_smooth  
        x_primary = self.fc1_primary(x)  # (B, F, 1, H/2)
        x_ghost_in = x_primary.permute(0, 3, 1, 2)
        x_ghost = self.fc1_ghost(x_ghost_in)
        x_ghost = x_ghost.permute(0, 2, 3, 1)  # (B, F, 1, H/2)
        x_hidden = torch.cat([x_primary, x_ghost], dim=-1)  # (B, F, 1, H)
        x_hidden = self.act(x_hidden)
        x_hidden = self.drop(x_hidden)
