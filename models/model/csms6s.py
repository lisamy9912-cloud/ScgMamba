...
try:
    import selective_scan_cuda_oflex as selective_scan_cuda_core
    print("SUCCESS: Loaded selective_scan_cuda_oflex", flush=True)
except Exception as e_oflex:
    print(f"WARNING: can not import selective_scan_cuda_oflex: {e_oflex}", flush=True)
    
    try:
        import selective_scan_cuda_core
    except Exception as e_core:
        print(f"WARNING: can not import selective_scan_cuda_core: {e_core}", flush=True)
...
class SelectiveScanCore(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, backnrows=1, oflex=True):
        ctx.delta_softplus = delta_softplus
        out, x, *rest = selective_scan_cuda_core.fwd(u, delta, A, B, C, D, delta_bias, delta_softplus, 1,True)
        ctx.save_for_backward(u, delta, A, B, C, D, delta_bias, x)
        return out
